...
@onready var villagebgm = $BGM/Villagebgm
@onready var forest = $BGM/Forrestbgm
...
func _on_forrest_body_entered(body):
#	if body.name == "Player":
#		$BGM/Forrestbgm.play()
	pass

func _on_village_body_exited(body):
	if body.name == "Player":
		if playing:
			#fade(0.0, villagebgm)
			playing = false
...
func _on_forrest_body_exited(body):
	if body.name == "Player":
		pass
		#$BGM/Forrestbgm.stop()
